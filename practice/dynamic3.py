...
class GraphFrame(wx.Frame):
    ...
    def draw_plot(self):
        ...
        self.plot_data.set_xdata(0.5*np.array([self.j-1, self.j]))
        self.plot_data.set_ydata(np.array(self.data1))

        self.canvas.draw()
        """
        self.axes.plot([(self.j-1.0), (self.j)],self.data1,color="green")
        self.canvas.draw()
    ...
    def on_redraw_timer(self, event):
        ...
        if not self.paused:
            ...
            self.data1[1] = self.datagen.next()
            self.j = time.time() - self.start
